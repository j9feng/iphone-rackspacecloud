...
@implementation SecureEditableCell
...
- (id)initWithFrame:(CGRect)frame reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithFrame:frame reuseIdentifier:reuseIdentifier]) {
		...
		labelField.textColor = [UIColor colorWithRed:0.222 green:0.326 blue:0.540 alpha:1.0];
...
- (void)layoutSubviews {
	...
	labelRect.size.width = 115;
	labelField.frame = labelRect;
	
	//textRect.origin.x += 90;
	textRect.origin.x += 115;
	//textRect.size.width -= 90; // to prevent scrolling off the side
	textRect.size.width -= 115; // to prevent scrolling off the side
